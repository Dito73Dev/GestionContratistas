...
function SignaturePad (selector, options) {
  ...
  function stopDrawing () {
    if (touchable) {
      ...
      canvas.off('mousemove.signaturepad')
    }
  ...
  function startDrawing (e, o) {
    if (touchable) {
      ...
      canvas.on('mousemove.signaturepad', drawLine)
    }
  ...
  function disableCanvas () {
    ...
    canvas.off('mousedown.signaturepad')
    canvas.off('mouseup.signaturepad')
    canvas.off('mousemove.signaturepad')
    canvas.off('mouseleave.signaturepad')

    $(settings.clear, context).off('click.signaturepad')
  }
  ...
  function initDrawEvents (e) {
    ...
    if (touchable) {
      ...
      canvas.on('mouseup.signaturepad', function (e) { stopDrawing() })
      canvas.on('mouseleave.signaturepad', function (e) {
  ...
  function drawIt () {
    ...
    canvas.on('mousedown.signaturepad', function (e) {
      initDrawEvents(e)
      startDrawing(e, this)
    })

    $(settings.clear, context).on('click.signaturepad', function (e) { e.preventDefault(); clearCanvas();
	var obj = $(this).closest("ul").next("div").find("canvas");
	fname = $(obj).attr("style");
	fname = fname.replace("background: url(","").replace(") no-repeat;","");
	$(obj).removeAttr("del");
	$(obj).attr("del",fname);
	$.get("signature-to-image.php", { 
		clear: $(".sigPad").attr("id")
	});
	if($(obj).attr("bgimage")){
		var attr = "url("+$(obj).attr("bgimage")+") no-repeat;";
		$(obj).attr("style", "background:"+attr); 
	}
	else
		$(obj).css("background","none"); 
	})

    $(settings.typeIt, context).on('click.signaturepad', function (e) { e.preventDefault(); typeIt() })
    $(settings.drawIt, context).off('click.signaturepad')
    $(settings.drawIt, context).on('click.signaturepad', function (e) { e.preventDefault() })
  ...
  function typeIt () {
    ...
    $(settings.drawIt, context).on('click.signaturepad', function (e) { e.preventDefault(); drawIt() })
    $(settings.typeIt, context).off('click.signaturepad')
    $(settings.typeIt, context).on('click.signaturepad', function (e) { e.preventDefault() })
  ...
  function onFormError (errors, context, settings) {
    if (errors.nameInvalid) {
      ...
      $(settings.name, context).trigger("focus")
  ...
  function init () {
    ...
    $(settings.typed, context).on('selectstart.signaturepad', function (e) { return $(e.target).is(':input') })
    canvas.on('selectstart.signaturepad', function (e) { return $(e.target).is(':input') })
    ...
    if (element.getContext) {
      ...
      if (!settings.displayOnly) {
        if (!settings.drawOnly) {
          $(settings.name, context).on('keyup.signaturepad', function () {
            type($(this).val())
          })

          $(settings.name, context).on('blur.signaturepad', function () {
            type($(this).val())
          })

          $(settings.drawIt, context).on('click.signaturepad', function (e) {
            e.preventDefault()
            drawIt()
          })
        }
        ...
        if (settings.validateFields) {
          if ($(selector).is('form')) {
            $(selector).on('submit.signaturepad', function () { return validateForm() })
          } else {
            $(selector).parents('form').on('submit.signaturepad', function () { return validateForm() })
          }
        }
